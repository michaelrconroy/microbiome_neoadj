...
analysisdir = "analysis/sev_irAE/LOESS/"
...
meta$high_grade_irae <- as.factor(meta$high_grade_irae)
names(meta)[names(meta) == "high_grade_irae"] <- "hgirae"
...
mycolors$hgirae["0"]          = "#DAA520"
mycolors$hgirae["1"]          = "#87CEEB"
...
for (mydist in dists[1]){
  p1 <- ggplot(meta, aes(x=timept,y=dist)) +
    geom_path(aes(group=Record_ID.1, color=hgirae), alpha=0.2) +
    geom_point(aes(color=hgirae), pch=19, size=2.5, alpha=0.4) +
    scale_fill_manual(values=mycolors$hgirae)  +
    scale_color_manual(values=mycolors$hgirae) +
  ...
  rDat  = meta[meta$dist.type==mydist & meta$hgirae == "1",]
  nrDat = meta[meta$dist.type==mydist & meta$hgirae == "0",]
  ...
  p1 <- ggplot(meta[meta$dist.type==mydist,], aes(x=timept,y=dist)) +
    ...
    geom_smooth(data=rDat,  method = 'loess', color="#87CEEB", fill="#87CEEB",  span=10, alpha=0.25) +
    geom_smooth(data=nrDat, method = 'loess', color="#DAA520", fill="#DAA520", span=10, alpha=0.25) +
    xlab("Days from IO Baseline") +
  ...
  bootStatsMelt$hgirae = toupper(gsub("\\.d.+$", "", bootStatsMelt$variable))
  ...
  p1 <- ggplot(bootStatsMelt, aes(x=hgirae,y=value)) +
    geom_boxplot(aes(color=hgirae), fill=NA, outlier.size=0, outlier.shape=NA, coef=1e100, alpha=0.6) +
    geom_point(aes(fill=hgirae), pch=21, color="black", alpha=0.6, size=2.5) +
    scale_fill_manual(values=mycolors$hgirae)  +
    scale_color_manual(values=mycolors$hgirae) +
    xlab(NULL) +
